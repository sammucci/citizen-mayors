...
import { deleteOrganizationAdmin, forceDeleteOrganizationAdmin } from "@/app/admin/actions";

// Same confirm / in-use / force-delete pattern as
// delete-decision-maker-button.tsx and delete-grant-button.tsx.
      ...
      <div className="flex items-center gap-2">
        ...
        <button
          ...
          onClick={async () => {
            ...
            fd.set("organization_id", id);
            const result = await deleteOrganizationAdmin(fd);
            if (result?.error) {
      ...
      {inUseCount != null && inUseCount > 0 && (
        <div className="mt-1 max-w-xs rounded-md border border-duty-red/30 bg-duty-red/5 p-2 text-right">
          <p className="text-[11px] text-neutral-600">
            ...
            detaches "{name}" from every resident's civic profile it's currently part of.
            There's no undo.
          </p>
          <div className="mt-1.5 flex items-center justify-end gap-1.5">
            ...
            <button
              ...
              onClick={async () => {
                ...
                const fd = new FormData();
                fd.set("organization_id", id);
                const result = await forceDeleteOrganizationAdmin(fd);
                setForcing(false);
                if (result?.error) {
                  setError(result.error);
                  return;
